...
def preprocessing_fn(inputs):
    ...
    raw_image_dataset = tf.map_fn(fn = lambda x : tf.io.parse_tensor(x[0], tf.uint8, name=None), elems = raw_image_dataset, fn_output_signature = tf.TensorSpec((100,100,3),dtype=tf.uint8,    name=None), infer_shape = True)
...
def _build_keras_model() -> tf.keras.Model:
    ...
    inputs = keras.Input(shape=(100,100,3), name='raw_image')
